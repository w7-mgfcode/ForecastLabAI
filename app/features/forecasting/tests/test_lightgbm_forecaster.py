"""Unit tests for ``LightGBMForecaster`` (PRP-30 / MLZOO-B).

The LightGBM forecaster is the first ADVANCED feature-aware model. Like
``RegressionForecaster`` it *consumes* the exogenous ``X`` argument, so these
tests mirror that contract: ``X`` is required, its shape is validated, fits are
deterministic, and ``NaN`` features are tolerated (LightGBM handles missing
values natively).

The whole module SKIPs (never ERRORs) when the optional ``ml-lightgbm``
dependency is absent — ``pytest.importorskip``. Importing ``LightGBMForecaster``
itself is leak-free (``lightgbm`` is imported lazily inside ``fit``), so the
...
from app.features.forecasting.models import LightGBMForecaster, model_factory
...
def test_fit_predict_roundtrip() -> None:
    """A fitted LightGBM model produces a finite forecast of horizon length."""
...
def test_fit_rejects_none_features() -> None:
    ...
    with pytest.raises(ValueError, match="requires exogenous features"):
        LightGBMForecaster().fit(target, None)
...
def test_fit_rejects_mismatched_rows() -> None:
    ...
    with pytest.raises(ValueError, match="rows must match"):
        LightGBMForecaster().fit(target, features[:-5])
...
def test_determinism_same_random_state() -> None:
    ...
    LightGBM is bit-reproducible only with ``n_jobs=1`` + ``deterministic=True``
    + ``force_col_wise=True`` — all pinned in ``fit`` — so an EXACT
    ``assert_array_equal`` is the correct gate.
    """
    features, target = _synthetic_data()
    future = features[:12]
    first = LightGBMForecaster(random_state=7).fit(target, features)
    second = LightGBMForecaster(random_state=7).fit(target, features)
    np.testing.assert_array_equal(first.predict(12, future), second.predict(12, future))


def test_handles_nan_features() -> None:
    """``LGBMRegressor`` tolerates NaN feature cells natively."""
    features, target = _synthetic_data()
    model = LightGBMForecaster().fit(target, features)
...
def test_get_and_set_params() -> None:
    ...
    model = LightGBMForecaster(n_estimators=150, learning_rate=0.03, max_depth=4)
...
def test_requires_features_is_true() -> None:
    """LightGBM is a feature-aware model — the ClassVar is True."""
    assert LightGBMForecaster.requires_features is True


def test_model_factory_creates_lightgbm_forecaster() -> None:
    """``model_factory`` dispatches a LightGBMModelConfig when the flag is on."""
    enabled = MagicMock()
    enabled.forecast_enable_lightgbm = True
    with patch("app.core.config.get_settings", return_value=enabled):
        model = model_factory(LightGBMModelConfig(n_estimators=120), random_state=42)
