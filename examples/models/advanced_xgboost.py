"""Example: Training and predicting with the XGBoost forecaster (MLZOO-C1).

``XGBoostForecaster`` is the second ADVANCED feature-aware tree model — it wraps
``xgboost.XGBRegressor`` and, like ``LightGBMForecaster``, REQUIRES an exogenous
feature matrix ``X`` for both ``fit`` and ``predict``.

XGBoost is an OPTIONAL dependency. Install the extra first:

    uv sync --extra dev --extra ml-xgboost

Usage:
    python examples/models/advanced_xgboost.py
"""
...
def main():
    ...
    model = XGBoostForecaster(n_estimators=100, learning_rate=0.1, max_depth=6, random_state=42)
    print(f"\nrequires_features: {XGBoostForecaster.requires_features}")

    # 3. Fit on the historical feature frame (``xgboost`` is imported lazily here).
